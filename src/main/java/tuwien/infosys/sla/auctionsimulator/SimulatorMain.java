	...
	private static Product generateRandomProduct(int paramAmount){
		...
		if(paramAmount==9){
			return ProductBuilder.newBuilder()
					.availability(SimulatorMain.RANDOM.nextInt(100-85)+85)
					.backupAvailable(SimulatorMain.RANDOM.nextBoolean())
					.bandwith(SimulatorMain.BANDWIDTH[SimulatorMain.RANDOM.nextInt(SimulatorMain.BANDWIDTH.length)])
					.cpu(SimulatorMain.CPU[SimulatorMain.RANDOM.nextInt(SimulatorMain.CPU.length)])
					.ipv6Support(SimulatorMain.RANDOM.nextBoolean())
					.latency(SimulatorMain.LATENCY[SimulatorMain.RANDOM.nextInt(SimulatorMain.LATENCY.length)])
					.operatingSystem(OperatingSystems.fromValue(SimulatorMain.RANDOM.nextInt(3)+1))
					.ram(SimulatorMain.RAM[SimulatorMain.RANDOM.nextInt(SimulatorMain.RAM.length)])
					.storage(SimulatorMain.STORAGE[SimulatorMain.RANDOM.nextInt(SimulatorMain.STORAGE.length)])
					.build();
		}
